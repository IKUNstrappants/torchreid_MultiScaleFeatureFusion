...
class Engine(object):
    ...
    def run(
        ...
        with open(f'{save_dir}/train_data.json', 'w') as file:
            json.dump({"loss": self.loss_cache, "result":self.test_result}, file)
    ...
    def _evaluate(
        ...
        self.test_result = 'mAP: {:.1%}'.format(mAP)
        print('CMC curve')
        for r in ranks:
            print('Rank-{:<3}: {:.1%}'.format(r, cmc[r - 1]))
            self.test_result += 'Rank-{:<3}: {:.1%}'.format(r, cmc[r - 1])
